...
export const getDesasignadosObjecion = async (projectoId, query) => {
  const URL = `${DOMAIN}/api/reporteProporcionDesasignadosByObjecion/${projectoId}`;
  const { data } = await axios.get(URL);
  return data;
};
export const getDesasignadosAsesor = async (projectId, query) => {
  const URL = `${DOMAIN}/api/reporteProporcionAsignadosDesasignadosByAsesor/${projectId}`;
  const { data } = await axios.get(URL);
  return data;
};

export const getDesasignadosEstadoLead = async (projectId, query) => {
  const URL = `${DOMAIN}/api/reporteProporcionDesasignadosByEstadoLead/${projectId}`;
  const { data } = await axios.get(URL);
  return data;
};
